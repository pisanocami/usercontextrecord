...
function getAIClient(): OpenAI | GoogleGenerativeAI | null {
  // Try OpenAI first
...
Provide your analysis as the ${council.name}.`;
      ...
      // Try to get AI client (OpenAI or Gemini)
      const aiClient = getAIClient();
      
      if (!aiClient) {
        console.warn("⚠️ No AI API Key found for council reasoning. Please set either OPENAI_API_KEY or GEMINI_API_KEY in .env");
        return null;
      }
      ...
      // Fallback to Gemini
      if (!responseText && aiClient instanceof GoogleGenerativeAI) {
        try {
          const model = aiClient.getGenerativeModel({ model: "gemini-1.5-pro" });
          
          const result = await model.generateContent([
            'You are an expert council member providing strategic analysis. Always respond with valid JSON.',
            prompt
          ]);
...
Respond as JSON with:
{
  ...
  "conflicts": [{ "topic": string, "councilA": string, "positionA": string, "councilB": string, "positionB": string, "resolution": string, "rationale": string }]
      ...
      if (!aiClient) {
        console.warn("⚠️ No AI API Key found for council synthesis. Please set either OPENAI_API_KEY or GEMINI_API_KEY in .env");
      ...
      if (!responseText && aiClient instanceof GoogleGenerativeAI) {
        try {
          ...
          const result = await model.generateContent([
            'You are the Master Synthesis Council. Provide unified strategic guidance.',
            synthesisPrompt
          ]);
